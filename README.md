...
- **Sıralı Yapıştırma:** `Cmd+Shift+C` ile birden fazla öğeyi sırayla kopyalayın. Ardından `Cmd+Shift+B` ile bu öğeleri kopyaladığınız sırayla farklı yerlere tek tek yapıştırın. Kuyruğu temizlemek için `Cmd+Shift+K` kullanın.
...
3. **Öğe Kopyalama:**
   ...
   - Öğenin üzerine gelin ve kopyala simgesine (`doc.on.doc`) tıklayarak panoya geri kopyalayın.
